...
let correctCount = 0;
let currentOptions = [];
const maxQuestionsPerSession = 24;
...
function getRandomUnusedIndex() {
  const avail = questions.map((_, i) => i).filter((i) => !usedIndices.has(i));
...
function updateCounter() {
  document.getElementById(
    "counter"
  ).textContent = `${usedIndices.size}/${maxQuestionsPerSession}`;
}
...
function showSummaryModal() {
  ...
  document.getElementById(
    "summary-text"
  ).textContent = `You answered ${correctCount} out of ${usedIndices.size} correctly. (${percentage}%)`;
